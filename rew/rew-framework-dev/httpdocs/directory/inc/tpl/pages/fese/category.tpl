...
if (!empty($refine_categories)) {
    echo '<div class="nav refine-search">';
    echo '<h3>Refine Your Search</h3>';
    echo '<ul class="kvs">';
    foreach ($refine_categories as $refine_category) {
        echo '<li class="kv">';
        echo '<a class="k" href="' . sprintf(URL_DIRECTORY_CATEGORY, $refine_category['link']) . '">' . Format::htmlspecialchars($refine_category['title']) . '</a>' . PHP_EOL;
        echo '<span class="label v">' . Format::number($refine_category['count']) . '</span>' . PHP_EOL;
        echo '</li>';
    }
    echo '</ul>';
    echo '</div>';
...
if (!empty($count_entries)) {
    ...
    // Display Listings
    if (!empty($entries)) {
        echo '<div class="articleset cols">';
...
if (!empty($related_categories)) {
    ...
    foreach ($related_categories as $related_category) {
        echo '<li class="kv">';
        echo '<a class="k" href="' . sprintf(URL_DIRECTORY_CATEGORY, $related_category['link']) . '">' . Format::htmlspecialchars($related_category['title']) . '</a>' . PHP_EOL;
        echo '<span class="label v">' . Format::number($related_category['count']) . '</span>' . PHP_EOL;
        echo '</li>';
    }
    echo '</ul>';
    echo '</div>';

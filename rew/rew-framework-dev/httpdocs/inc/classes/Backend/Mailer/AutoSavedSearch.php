...
/**
 * Backend_Mailer_AutoSavedSearch
 *
 * @package Backend
 */
class Backend_Mailer_AutoSavedSearch extends Backend_Mailer
{
    ...
    public function getMessage(&$tags = array())
    {
        ...
        $tags = array_merge($tags, array(
            ...
            'url'        => $url,
            'uid'        => $uid,
            'url_edit'   => $url . 'idx/search.html?' . http_build_query(array(
                'edit_search'     => true,
                'uid'             => $uid,
                'saved_search_id' => $this->data['search_id'],
                'lead_id'         => $lead['id']
            )),
            'signature'    => $signature,
        ...
        $this->message .= '<p>If you would like to refine this saved search\'s criteria further, simply click <a href="{url_edit}">Edit this Search</a>!' . PHP_EOL;

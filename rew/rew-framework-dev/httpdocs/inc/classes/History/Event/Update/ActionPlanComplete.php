...
/**
 * History_Event_Update_ActionPlanComplete
 *
 * <code>
 * </code>
 *
 */
class History_Event_Update_ActionPlanComplete extends History_Event_Update
{
    ...
    function getMessage(array $options = array())
    {
        ...
        if ($options['view'] == 'system') {
            return $agent->displayLink() . ' Completed Action Plan for ' . $lead->displayLink() . ': ' . $this->getData('action_plan');
        }

        // Agent/Associate History
        if (in_array($options['view'], array('agent', 'associate', 'lender'))) {
            return 'Completed Action Plan for ' . $lead->displayLink() . ': ' . $this->getData('action_plan');
        }

        // Lead History
        if ($options['view'] == 'lead') {
            return $agent->displayLink() . ' Completed Action Plan: ' . $this->getData('action_plan');
        }
    }
}

...
/**
 * History_Event_Create_LeadTransaction extends History_Event_Create and is used for tracking when an Agent creates a lead transaction.
 *
 * <code>
 * try {
 *
 ...
 * @package History
 */
class History_Event_Create_LeadTransaction extends History_Event_Create implements History_IExpandable
{
    ...
    function getMessage(array $options = array())
    {
        ...
        if ($options['view'] == 'system') {
            return $agent->displayLink() . ' added ' . $type . ' transaction for ' . $lead->displayLink();
        }

        // Agent/Associate/Lender History
        if (in_array($options['view'], array('agent'))) {
            return $type . ' transaction added for ' . $lead->displayLink();
        }

        // Lead History
        if ($options['view'] == 'lead') {
            return $type . ' transaction added by ' . $agent->displayLink();
        }
    }

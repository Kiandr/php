...
/**
 * History_Event_Phone_Attempt
 *
 * <code>
 * </code>
 *
 */
class History_Event_Phone_Attempt extends History_Event_Phone
{
    ...
    function getMessage(array $options = array())
    {
        ...
        if ($options['view'] == 'system') {
            return $agent->displayLink() . ' Attempted Phone Call to ' . $lead->displayLink();
        }

        // Agent/Associate/Lender History
        if (in_array($options['view'], array('agent', 'associate', 'lender'))) {
            return 'Phone Call Attempt to ' . $lead->displayLink();
        }

        // Lead History
        if ($options['view'] == 'lead') {
            return 'Call Attempt by ' . $agent->displayLink();
        }
    }
}

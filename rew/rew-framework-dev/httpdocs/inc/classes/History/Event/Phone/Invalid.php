...
/**
 * History_Event_Phone_Invalid
 *
 * <code>
 * </code>
 *
 */
class History_Event_Phone_Invalid extends History_Event_Phone
{
    ...
    function getMessage(array $options = array())
    {
        ...
        if ($options['view'] == 'system') {
            return $agent->displayLink() . ' flagged Bad Phone Number for ' . $lead->displayLink();
        }

        // Agent/Associate/Lender History
        if (in_array($options['view'], array('agent', 'associate', 'lender'))) {
            return 'Flagged Bad Phone Number for ' . $lead->displayLink();
        }

        // Lead History
        if ($options['view'] == 'lead') {
            return 'Bad Phone Number flagged by ' . $agent->displayLink();
        }
    }
}

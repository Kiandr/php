...
/**
 * History_Event_Phone_Voicemail
 *
 * <code>
 * </code>
 *
 */
class History_Event_Phone_Voicemail extends History_Event_Phone
{
    ...
    function getMessage(array $options = array())
    {
        ...
        if ($options['view'] == 'system') {
            return $agent->displayLink() . ' left Voicemail for ' . $lead->displayLink();
        }

        // Agent/Associate/Lender History
        if (in_array($options['view'], array('agent', 'associate', 'lender'))) {
            return 'Voicemail left for ' . $lead->displayLink();
        }

        // Lead History
        if ($options['view'] == 'lead') {
            return 'Voicemail left by ' . $agent->displayLink();
        }
    }
}

...
class phpthumb {
	...
	var $config_command_base                         = 'convert';
	...
	var $config_http_user_agent                      = false;
...
//		}
	...
	function RenderToFile($filename) {
		...
		if ($this->RenderOutput()) {
			if (file_put_contents($renderfilename, $this->outputImageData)) {
			    ...
			    // If the file is a PNG optimize the file using OptiPNG
			    if (preg_match('#png#i', $this->thumbnailFormat)) {
			        exec("optipng -strip all -backup " . escapeshellarg($renderfilename) . " -clobber " . escapeshellarg($renderfilename));
			    }
	...
	function ImageMagickCommandlineBase($true_base = false) {
		static $cmd = array();
		if (is_null($cmd[$true_base])) {
			if ($this->issafemode) {
				$$cmd[$true_base] = '';
				return $cmd[$true_base];
			}
			...
			if ($this->file_exists_ignoreopenbasedir($this->config_imagemagick_path)) {
				...
				if ($which_convert && ($which_convert{0} == '/') && $this->file_exists_ignoreopenbasedir($which_convert)) {
					...
					if ($true_base) {
						$commandline = $this->config_command_base;
					}
					else {
						// Steven's Memory Capped Command
						// @todo: change/remove in the future, as images are now really large and high resolution.
						// Retained from:
						//   Author: REWStevenBarre <steven@realestatewebmasters.com>
						//   Date:   Fri Apr 10 16:37:38 2015 +0000
						//   Improving r7879 to set imagick memory limits. Turns out I set it to bytes not megabytes ...
						//   git-svn-id: https://www.realestatewebmasters.com/svn/rew-framework/trunk@9021 01cb2065-007e-0410-b27c-8de3a837154f
						$commandline = 'MAGICK_TMPDIR=' . $this->config_temp_directory . ' '.$this->config_command_base.' -limit memory 16MiB -limit map 32MiB';
					}
	...
	function ImageMagickVersion($returnRAW=false) {
		...
		if (is_null($versionstring)) {
			...
			if ($cachedversionstring = @file_get_contents($IMversionCacheFilename)) {
				...
				$commandline = $this->ImageMagickCommandlineBase(true);
...
//				return $this->ErrorImage('Unknown image type identified by "'.substr($this->rawImageData, 0, 4).'" ('.phpthumb_functions::HexCharDisplay(substr($this->rawImageData, 0, 4)).') in SourceImageToGD()['.__LINE__.']');
	...
	function InitializeTempDirSetting() {
		...
		if (empty($this->config_temp_directory)) {
			$this->config_temp_directory = realpath(!empty(ini_get('upload_tmp_dir')) ? ini_get('upload_tmp_dir') : sys_get_temp_dir());
		}
		return true;
	}

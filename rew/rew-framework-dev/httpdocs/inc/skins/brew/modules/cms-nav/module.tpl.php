	...
	<div<?=(!empty($nav['class']) ? ' class="' . Format::htmlspecialchars($nav['class']) . '"' : ''); ?>>
		...
		<ul class="nav">
			<?php foreach ($nav['pages'] as $page) { ?>
				...
				<li<?=(!empty($current) ? ' class="current"' : ''); ?>><a href="<?=$page['link']; ?>"<?=!empty($page['target']) ? ' target="' . $page['target'] . '"' : ''; ?>><?=$page['title']; ?></a>
					<?php if (!empty($page['subpages'])) { ?>
						<ul>
							<?php foreach ($page['subpages'] as $subpage) { ?>
								...
								<li<?=(!empty($current) ? ' class="current"' : ''); ?>><a href="<?=$subpage['link']; ?>"<?=!empty($subpage['target']) ? ' target="' . $subpage['target'] . '"' : ''; ?>><?=$subpage['title']; ?></a></li>
							<?php } ?>
						</ul>
					<?php } ?>
				</li>

...
(function () {
	...
	REWMap.loadApi(function () {
		...
		var predictLocation = function (input, callback) {
			...
			service.getPlacePredictions({
				...
				if (status != google.maps.places.PlacesServiceStatus.OK) {
					$message.html('Your location "' + $('<span />').text(input).html() + '" could not be found. Please try another address.').removeClass('hidden');
					//alert(status);
					return;
				}
		...
		var setLocation = function (place) {
			...
			var _markers = [{
				...
				'lat'			: place.lat,
				'lng'			: place.lng,
				'icon'			: '<?=str_replace($_SERVER['DOCUMENT_ROOT'], '', __DIR__); ?>/img/map-flag.png',
				'iconWidth'		: 22,
				'iconHeight'	: 25,
				'zIndex'		: 1000
			}];
			...
			$map = $module.find('.map').REWMap({
				...
				'onInit' : function () {
					...
					var $mapTools = $module.find('.map-controls').removeClass('hidden'), mapTools = $mapTools.get(0);
		...
		$form.on('change', function () {
			...
			if (xhr) xhr.abort();
			xhr = $.ajax({
				'url' : '/idx/inc/php/ajax/html.php?module=<?=$this->getID(); ?>',
				'dataType' : 'json',
				'type' : 'get',
				'data' : {
					'feed'	: $feed.val(),
					'options' : {
						...
						'defaults.location'	: <?=json_encode($this->config('defaults.location')); ?>
					}
				},
				'success' : function (data) {
					if (data) {
						...
						$results.find('.listings').html(results);
						$results.removeClass('hidden');
						if (typeof BREW !== 'undefined') {
							$results.Images({ onLoad : false });
						}
						$results.find('.view-all').attr('href', data.url).text('See all ' + shortNumber(total) + ' matches').toggleClass('hidden', (data.total < 2));
		...
		$.ajax({
			...
			'success'	: function (html) {
				$.Window({
					'width'		: 750,
					'title'		: title,
					'content'	: '<div class="cma-popup-container popup-container">' + html + '</div>',
					'onOpen'	: function (win) {
						...
						$el.on('submit', 'form', function () {
							$.ajax({
								...
								'success'	: function (html) {
									win.setContents('<div class="cma-popup-container popup-container">' + html + '</div>');
								}
							});
							return false;
						});
	...
	<?php if ($this->config('capture') && !User_Session::get()->isValid() && !Auth::get()->isValid()) { ?>

		// Bring up the require form on refine
		var $refine = $form.find('.body-b').on('change', function () {
			...
			// Submit cma form
			$.ajax({
				'url'		: '/idx/inc/php/ajax/html.php?module=cms-snippet&options[name]=form-cma-capture',
				'type'		: 'post',
				'data'		: {
					'address' : $input.val()
				},
				'success'	: function (html) {
					$.Window({
						'width'		: 750,
						'title'		: 'Get a Free In-Person Assessment',
						'content'	: '<div class="cma-popup-container popup-container">' + html + '</div>',
						'noClose'	: true,
						'onOpen'	: function (win) {
							var $el = win.getWindow();
							$el.on('submit', 'form', function () {
								$.ajax({
									'url'		: '/idx/inc/php/ajax/html.php?module=cms-snippet&options[name]=form-cma-capture&submit',
									'type'		: 'post',
									'data'		: $(this).serialize() + '&' + data,
									'success'	: function (html) {
										win.setContents('<div class="cma-popup-container popup-container">' + html + '</div>');
										// Successful form submission (this is not the best method of detection - but it gets the job done)
										if (html.indexOf('msg success') !== -1) {
											//window.location = window.location.href.split('?')[0] + '?' + $form.serialize();
											setTimeout(function () {
												$refine.off('change');
												win.close();
											}, 1000);
	...
	var fixHeight = function () {
		$module.find('.map').height($module.find('.section-b').height() - $module.find('.estimate-values').height());
	}; fixHeight();
	...
	$form.find('select[name="type"]').on('change', function () {
		...
		if ($subtypes.length) {
			// No type selected
			if (value.length === 0) {
				$subtypes.closest('.field, .fld').addClass('hidden');
				...
				'success' : function (json) {
					...
					if (json.returnCode == 200) {
						...
						if (json.options.length === 0) {
							...
							$subtypes.closest('.field, .fld').removeClass('hidden');

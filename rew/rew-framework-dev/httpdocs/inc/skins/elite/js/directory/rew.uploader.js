...
(function($) {
    ...
    var docTypes = ['doc', 'docx', 'rtf', 'pdf', 'txt', 'xls', 'xlsx', 'odt'];
...
;
        ...
        _addUpload : function (upload) {
            ...
            if (o.extraParams.upload == 'img') {
                var $img   = $('<img />', { alt: '', src: '/thumbs/95x95' + upload.file + '?' + new Date().valueOf()});
            } else if (o.extraParams.upload == 'doc') {
                var $img   = $('<img />', { alt: '', src: '/img/icons/blank-doc.png' });
            } else {
                var $img = $('<span class="file ico-' + upload.ext + '">' + upload.name + '</span>');
            }
        ...
        _setupActions : function () {
            ...
            var self = this, o = self.options, $list = self.manager.find('ul:first');

            /* Action Buttons (Edit/View/Delete) */
            var actions = [];
            if (o.buttons['edit'])   actions.push('<a href="#" class="ui-button edit">Edit</a>');
            if (o.buttons['view'])   actions.push('<a href="#" class="ui-button view">View</a>');
            if (o.buttons['delete']) actions.push('<a href="#" class="ui-button delete">Delete</a>');
...
;
	            ...
	            /* Delete Action */
	            $actions.find('.delete').bind('click', function (e) {
	            	if (self._trigger('onDelete', null, {
                		el : $item,
                		upload : upload,
                	}) === false) {
...
;
        ...
        getUploads: function () {
            var uploads = []
                ...
                , $inputs = this.manager.find(':input[type="hidden"][name="' + inputName + '"]')
;

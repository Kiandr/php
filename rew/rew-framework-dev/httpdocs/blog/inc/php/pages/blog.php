...
// Count Blog Entries
$query = $db->query("SELECT COUNT(`id`) AS `total` FROM `" . TABLE_BLOG_ENTRIES . "` WHERE `published` = 'true' AND `timestamp_published` < NOW()" . (Settings::getInstance()->SETTINGS['agent'] != 1 ? " AND `agent` = " . $db->quote(Settings::getInstance()->SETTINGS['agent']) : "") . " ORDER BY `timestamp_published` DESC;");
...
if (!empty($count_entries)) {
    ...
    if ($count_entries > BLOG_PAGE_LIMIT) {
        ...
        } else if (isset($_GET['p']) && $_GET['p'] > 0) {
            $page_title = $page_title . ' | Page #' . (int) $_GET['p'];
        }
    ...
    // Select Blog Entries, Order Latest Published First
    $entries = array();
    $result = $db->query("SELECT * FROM `" . TABLE_BLOG_ENTRIES . "` WHERE `published` = 'true' AND `timestamp_published` < NOW()" . (Settings::getInstance()->SETTINGS['agent'] != 1 ? " AND `agent` = " . $db->quote(Settings::getInstance()->SETTINGS['agent']) : "") . " ORDER BY `timestamp_published` DESC" . $sql_limit . ";");
    while ($entry = $result->fetch()) {
        ...
        // Remove All Other Blog Post Images But The First
        if (preg_match('/<img.*?>/', $entry['body'], $matches, PREG_OFFSET_CAPTURE)) {
            $str_start = substr($entry['body'], 0, $matches[0][1]+1);
            $str_end = substr($entry['body'], $matches[0][1] + 1);
            $str_end = preg_replace('/<img.*?>/', '', $str_end);
            $entry['body'] = $str_start . $str_end;
        }

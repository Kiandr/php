...
if (!function_exists('armls_parse_listing')) {
	function armls_parse_listing (IDX $idx, Database $db_idx, $listing) {
		if (empty($listing['ListingPrice']) && !empty($listing['ListingPriceMin']) && !empty($listing['ListingPriceMax'])) {
			$listing['ListingPrice'] = Format::number($listing['ListingPriceMin']).' - $'.Format::number($listing['ListingPriceMax']);
		}
...
if (!function_exists('armls_search_fields')) {

    function armls_search_fields($idx, $filter = '', $comparison = false)  {
        ...
        $IDX_SEARCH_FIELDS = array(
            ...
            array('name' => 'Min. Price',          'form_field' => 'minimum_rent',          'idx_field' => 'ListingPriceMin',       'match' => 'morethan'),
            array('name' => 'Max. Price',          'form_field' => 'maximum_rent',          'idx_field' => 'ListingPriceMax',       'match' => 'lessthan'),

...
if (!function_exists('cbrga_parse_listing')) {

	function cbrga_parse_listing (IDX $idx, Database $db_idx, $listing) {

		// Listing Sitemap Compliance
		if ($_GET['load_page'] === 'sitemap') {
			$provider = array();
			if (!empty($listing['ListingAgent'])) $provider[] = $listing['ListingAgent'];
			if (!empty($listing['ListingOffice'])) $provider[] = $listing['ListingOffice'];
			$provider = implode(', ', $provider);
			$listing['ListingAgent'] = $provider;
...
if (!function_exists('cbrga_search_fields')) {

	function cbrga_search_fields($idx, $filter = '', $comparison = false)  {

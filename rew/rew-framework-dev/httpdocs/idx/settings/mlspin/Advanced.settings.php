...
if (!function_exists('mlspin_parse_listing')) {
	function mlspin_parse_listing (IDX $idx, Database $db_idx, $listing) {

		// Compliance Retricted Items
		global $user;
		if (!$user->isValid()) {

			// Strip data if user not logged in
			$no_data = array(
                'HasBasement',
                'NumberOfFireplaces',
                'DescriptionAppliances',
                'DescriptionInteriorFeatures',
                'DescriptionAmenities',
                'DescriptionConstruction',
                'DescriptionExterior',
                'DescriptionExteriorFeatures',
                'DescriptionFoundation',
                'DescriptionHeating',
                'DescriptionCooling',
                'DescriptionLot',
                'DescriptionRoofing',
                'DescriptionWaterfront',
                'DescriptionBeach',
                'DescriptionElectric',
                'DescriptionEnergyFeatures',
                'DescriptionInsulationFeatures',
                'DescriptionFlooring',
                'DescriptionFoundationSize',
                'DescriptionHotWater',
                'DescriptionRoadType',
                'DescriptionSewerAndWater',
                'DescriptionSewageDistrict',
                'DescriptionTermsFeature',
                'DescriptionUtilityConnections',
                'HasBasement'
			);

			foreach ($listing as $key => $val) {
				if (in_array($key, $no_data) && !empty($listing[$key])) {
					$listing[$key] = '';
				}
			}
...
if (!function_exists('mlspin_search_fields')) {

	function mlspin_search_fields($idx, $filter = '', $comparison = false)  {

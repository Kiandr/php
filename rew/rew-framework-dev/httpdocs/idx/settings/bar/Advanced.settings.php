...
if (!function_exists('bar_parse_listing')) {
	function bar_parse_listing (IDX $idx, Database $db_idx, $listing) {

		// Compliance: Provider Details
		if (isset($listing['ListingOffice'])) {

			$listing['ListingOffice'] = $listing['ListingAgent'] . ', '
				. $listing['ListingOffice'];

			if (in_array($_GET['load_page'], array('details','brochure'))) {
				$listing['ListingOffice'] .= ', '
					. $listing['ListingOfficeAddress'] . ', '
					. $listing['ListingOfficeCity'] . ' '
					. $listing['ListingOfficeZipCode'] . ' '
					. $listing['ListingOfficePhone'];
			}
		}
...
if (!function_exists('bar_search_fields')) {

	function bar_search_fields($idx, $filter = '', $comparison = false)  {

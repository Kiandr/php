...
if (!function_exists('ivbor_parse_listing')) {
	function ivbor_parse_listing (IDX $idx, Database $db_idx, $listing) {
		// Compliance
		global $_COMPLIANCE;
		// Listing Sitemap Compliance
		if ($_GET['load_page'] === 'details' && !empty($listing['ListingOffice']) && !empty($listing['ListingAgent'])) {
			$listing['ListingOffice'] = 'Listing Courtesy of ' . $listing['ListingAgent'] . ' of ' . $listing['ListingOffice'] . '<br>';
		}
...
if (!function_exists('ivbor_search_fields')) {

	function ivbor_search_fields($idx, $filter = '', $comparison = false)  {

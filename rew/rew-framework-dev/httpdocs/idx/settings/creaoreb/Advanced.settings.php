...
if (!function_exists('creaoreb_images')) {
    function creaoreb_images (IDX $idx, Database $db_idx, $listing, $thumbnails = false) {
        $images = array();
        if ($thumbnails) {
            $thumbnails = $db_idx->query("SELECT SQL_CACHE `ImageURL`  FROM `_rewidx_images` WHERE `ListingMLS` = '" . $db_idx->cleanInput($listing['ListingUniqueID']) . "' ORDER BY `ImageOrder` ASC");
            if ($thumbnails) {
                while ($thumb = $db_idx->fetchArray($thumbnails)) {
                    if (!empty($thumb['ImageURL'])) {
                        $images[] = $thumb['ImageURL'];
                    }
                }
            }
        } else {
        ...
        $images = !empty($images) ? $images : array(Settings::getInstance()->SETTINGS['URL_IMG'] . 'no-image.jpg');
        return $images;
    }
}

if (!function_exists('creaoreb_search_fields')) {

    function creaoreb_search_fields($idx, $filter = '', $comparison = false)  {
        ...
        $IDX_SEARCH_FIELDS = array(
            ...
            array('name' => 'Subdivision',         'form_field' => 'search_subdivision',    'idx_field' => 'AddressSubdivision',    'match' => 'like'),

...
if (!function_exists('fmls_parse_listing')) {
    function fmls_parse_listing (IDX $idx, Database $db_idx, $listing) {

        // NULLify Sq. Ft. if not viewing details or brochure
        if (!in_array($_GET['load_page'], array('details', 'brochure'))) {
			$listing['NumberOfSqFt'] = null;
        }

		// If No Sq.Ft. NULLify source
        if (empty($listing['NumberOfSqFt'])){
			$listing['DescriptionSqFtSource'] = null;
        }

		if ((empty($_REQUEST['pid']) || $listing['ListingMLS'] == $_REQUEST['pid'])
				&& !empty($listing['DescriptionSqFtSource'])) {
			Lang::$lang['MAP_POPUP_SQFT_TEXT'] = ' sqft (Source: ' . $listing['DescriptionSqFtSource'] . ')';
			Lang::$lang['IDX_DETAILS_SQFT_TEXT'] = 'SQ. Feet (Source: ' . $listing['DescriptionSqFtSource'] . ')';
		}

		if (in_array($_GET['load_page'], array('birdseye', 'streetview', 'local'))) {
			unset($listing['ListingPrice'], $listing['Address'], $listing['AddressCity'], $listing['AddressState'], $listing['ListingOffice']);
		}
...
if (!function_exists('fmls_search_fields')) {

    function fmls_search_fields($idx, $filter = '', $comparison = false)  {

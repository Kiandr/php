...
if (!function_exists('cbroh_parse_listing')) {
	function cbroh_parse_listing (IDX $idx, Database $db_idx, $listing) {
		// Return Empty Array
		if (empty($listing['ListingMLS'])) return array();

		// Format ListingOffice & ListingAgent
		$listing['ListingOffice']  = (isset($listing['ListingAgent']) ?
				ucwords(strtolower($listing['ListingAgent']))  : '')
			. (isset($listing['ListingAgentPhoneNumber']) ?
				' ' . strtolower($listing['ListingAgentPhoneNumber']) : '')
			. ' of ' . (isset($listing['ListingOffice']) ?
				ucwords(strtolower($listing['ListingOffice'])) : '')
			. (isset($listing['ListingOfficePhoneNumber']) ?
				' ' . strtolower($listing['ListingOfficePhoneNumber']) : '');
...
if (!function_exists('cbroh_search_fields')) {

	function cbroh_search_fields($idx, $filter = '', $comparison = false)  {

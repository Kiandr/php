...
if (!function_exists('metrolistco_parse_listing')) {

	function metrolistco_parse_listing (IDX $idx, Database $db_idx, $listing) {

		// Compliance
		global $_COMPLIANCE;

		// LEC2013 : Prepend logo to details office
		if (Skin::getClass() == 'Skin_Lec2013') {
			if ($_GET['load_page'] === 'details' && !empty($listing['ListingOffice'])) {
				$listing['ListingOffice'] = $_COMPLIANCE['results']['show_icon'] . ' ' . $listing['ListingOffice'];
			}
		}

		// Listing Sitemap Compliance
		if ($_GET['load_page'] === 'sitemap' && !empty($listing['ListingOffice'])) {
			$listing['ListingOffice'] = '<strong> ' . $listing['ListingOffice'] . '</strong><br>';
		}
...
if (!function_exists('metrolistco_search_fields')) {

    function metrolistco_search_fields($idx, $filter = '', $comparison = false)  {

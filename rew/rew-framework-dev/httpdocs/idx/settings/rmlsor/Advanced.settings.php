...
if (!function_exists('rmlsor_parse_listing')) {
	function rmlsor_parse_listing (IDX $idx, Database $db_idx, $listing) {
		if (!empty($listing['ListingPriceMin']) && !empty($listing['ListingPriceMax'])) {
			if (in_array($_GET['load_page'], array('details', 'brochure'))) { 
				...
				$listing['ListingPrice'] = $listing['ListingPriceMax'];
			}
		}
...
if (!function_exists('rmlsor_search_fields')) {

    function rmlsor_search_fields($idx, $filter = '', $comparison = false)  {

...
<?php if (!empty($history)) { ?>
	<div id="listing-history">
		<h3>Price Change History for <?=$listing['Address']; ?>, <?=$listing['AddressCity']; ?>, <?=$listing['AddressState']; ?> (<?=Lang::write('MLS_NUMBER'); ?><?=$listing['ListingMLS']; ?>)</h3>
		<table>
			<thead>
				<tr>
					<th>Date</th>
					<th>Details</th>
					<th class="hidden-phone">Change</th>
				</tr>
			</thead>
					...
					<?php foreach ($history as $j => $change) { ?>
						<tr>
							...
							<?php if ($change['Type'] === 'Price') { ?>
								<td class="<?=($change['Diff'] < 0 ? 'neg': 'pos'); ?> hidden-phone">
							...
							<?php } else { ?>
								<td class="hidden-phone">&ndash;</td>
							<?php } ?>
						</tr>
					<?php } ?>

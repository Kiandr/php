...
if (empty($listing)) {
	...
	if (empty($location)) {
	    echo sprintf(
            '<div class="notice notice--negative"><div class="notice__message">%s.</div></div>',
            'This feature is currently unavailable in this listing\'s region.'
        );
...
?>
	...
	<div class="tabbed-content">
		...
		<div class="nav nav--tabs -clear -mar-vertical-sm">
			<ul class="nav__list -text-xs">
				<li<?=($view == 'nearby-amenities')			? ' class="nav__item -is-current" ' : ' class="nav__item" '; ?>><a class="nav__link" href="?view=nearby-amenities" data-panel="#nearby-amenities">Nearby Amenities</a></li>
				<li<?=($view == 'nearby-schools')			? ' class="nav__item -is-current" ' : ' class="nav__item" '; ?>><a class="nav__link" href="?view=nearby-schools" data-panel="#nearby-schools">Nearby Schools</a></li>
				<li<?=($view == 'community-information')	? ' class="nav__item -is-current" ' : ' class="nav__item" '; ?>><a class="nav__link" href="?view=community-information" data-panel="#community-information">Neighborhood Information</a></li>
			</ul>
		</div>
						...
						<div>
							<?php foreach ($statistics as $statistic) { ?>
								<div class="keyvals">
									...
									<?php } ?>
									<ul class="keyvals__body">
										<?php foreach ($statistic['statistics'] as $stats) { ?>
											<li class="keyval">
												<strong class="keyval__key"><?=$stats['title']; ?>:</strong>
												<span class="keyval__val"><?=$stats['value']; ?></span>
											</li>
										<?php } ?>
									</ul>
								</div>
...
/* <script> */
	...
	var $map = $('#idx-map-onboard').REWMap($.extend(<?=$map; ?>, {
		onInit : function () {
			...
			// Activate Tab
			var $active = $tabs.find('li.-is-current a');
	...
	// Toggle Tabs
	var $tabs = $('div.tabbed-content .nav--tabs').on(BREW.events.click, ' a', function () {
		var $this = $(this), $item = $this.parent('li');
		if (!$item.hasClass('-is-current')) {
			// Toggle Panel
			var panel = $this.data('panel'), $panel = $(panel);
			if ($panel.length > 0) {
				$item.addClass('-is-current').siblings('li').removeClass('-is-current');
			...
			$map.REWMap(toggle, function () {
				...
				$panel.html('<div class="notice"><p>Loading Results...</p></div>');
				// Load Panel...
				$.ajax({

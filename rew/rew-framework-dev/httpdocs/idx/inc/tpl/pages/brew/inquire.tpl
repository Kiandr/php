...
?>
	...
	// Otherwise Default to REW Form
		...
		<form action="?submit" method="post" id="brew_tpl_inquire">
			...
			<div class="field x6 last">
				<label>Phone <?=empty(Settings::getInstance()->SETTINGS['registration_phone']) ? '<small>(optional)</small>' : ''; ?></label>
				<input type="tel" name="phone" value="<?=htmlspecialchars($_POST['phone']); ?>"<?=!empty(Settings::getInstance()->SETTINGS['registration_phone']) ? ' required' : ''; ?>>
			...
			<div class="btnset">
				<button type="submit" class="strong"><?=($_REQUEST['inquire_type'] == 'Property Showing') ? 'Send Request' : 'Send Inquiry'; ?></button>
			</div>

...
// Check one
const $check = $('input[name="pings[]"]').on('change', function () {
...
const $delete = $('#btn-delete').on('click', function () {
    ...
    if (confirm ('Are you sure you want to delete the selected blog pingbacks?')) {
...
const $publish = $('#btn-publish').on('click', function () {
    const $form = $(this).closest('form');
    if (confirm ('Are you sure you want to publish the selected blog pingbacks?')) {

    ...
    <div class="flyout-filter" :class="{'-is-flyout-open' : is_open}">
        ...
        <div>
            ...
            <header class="flyout-header">
                <h2 class="title">Filter Blog Posts</h2>
            </header>
            <form @submit.prevent="submitFilterBlogPosts">
                <div class="flyout-content">
                    <div>
                        <div class="-mar-bottom">
                            <label class="field__label">Entry Title</label>
                            <input type="text" v-model="title" name="title" @input="clearInvalidWarnings" />
                        </div>
                        <div class="-mar-bottom">
                            <label class="field__label">Author First Name</label>
                            <input type="text" v-model="first_name" name="first_name" @input="clearInvalidWarnings" />
                        </div>
                        <div class="-mar-bottom">
                            <label class="field__label">Author Last Name</label>
                            <input type="text" v-model="last_name" name="last_name" @input="clearInvalidWarnings" />
                        </div>
                        <div class="-mar-bottom">
                            <label class="field__label">Tags</label>
                            <input type="text" v-model="tags" name="tags" @input="clearInvalidWarnings" />
                        </div>
                    </div>
                </div>
                <div>
...
<script>
    ...
    export default {
        ...
        data: function () {
            const { title, first_name, last_name, tags } = this.$route.query;
            return {
                title: title ? title : '',
        ...
        methods: {
            submitFilterBlogPosts: function () {
                if (window.innerWidth <= 500) {
                    sessionStorage.removeItem('isOpenFilterBlogPosts');
                }
                window.location.href = '?search=' + this.queryStringFilterBlogPosts();
            },
            queryStringFilterBlogPosts: function () {
                let queryStringPages = '';
                if (this.title !== '') {
                    queryStringPages += '&title=' + encodeURI(this.title.trim());
                }
                if (this.first_name !== '') {
                ...
                if (this.tags !== '') {
                    queryStringPages += '&tags=' + encodeURI(this.tags.trim());
                }
        ...
        computed: {
            is_open: function () {
                ...
                let isOpenSession = sessionStorage.getItem('isOpenFilterBlogPosts');
                if (isOpenSession) {
                    store.dispatch(`flyouts/${'open'}`, {
                        flyout: 'Filter'
                    });
                } else if (isOpen) {
                    sessionStorage.setItem('isOpenFilterBlogPosts', true);
                }

...
class EnableController extends AbstractController
{
    ...
    public function __invoke()
    {
        ...
        // Handle request to enable field
        if ($this->checkSubmitting()) {
            try {
                // Enable Field
                $this->enableField($customField);

                // Set success message and save notifications
                $this->notices->success(__('%s has successfully been enabled.', $customField->getTitle()));

                // Redirect to fields list
                header('Location: ' . URL_BACKEND . 'settings/fields');
                exit;
            } catch (PDOException $e) {
                $this->notices->error(__('Custom field could not be enabled.'));
                $this->log->error($e);
        ...
        echo $this->view->render('::partials/field/summary.tpl.php', [
            'title' => 'Enable ' . $this->format->htmlspecialchars($customField->getTitle()),
        ...
        echo $this->view->render('::pages/settings/fields/field/enable', [
     ...
     */
    public function enableField(CustomFieldInterface $customField)
    {
        ...
        // Create Update Query
        $enableQuery = $this->db->prepare("UPDATE `" . $this->getTable(). "` SET `enabled`  = 1 WHERE `id` = :id");

		...
		try {
			...
			if ($total == -1 || $loaded + $blobSize == $total) {
				...
				if ($config->get("upload.autoresize") && preg_match('/gif|jpe?g|png/i', MOXMAN_Util_PathUtils::getExtension($tempFilePath)) === 1) {
					...
					$quality = $config->get('upload.autoresize_jpeg_quality');

					if ($size[0] > $maxWidth || $size[1] > $maxHeight) {
						switch (MOXMAN_Util_PathUtils::getExtension($tempFilePath)) {
							case "jpg":
							case "jpeg":
								// Use ImageMagick To Compress Image, Convert To Progressive, and Resize as Necessary
								exec("convert -strip -interlace line -quality " . escapeshellarg($quality) . "% -resize " . $maxWidth . "x" . $maxHeight . " " . escapeshellarg($tempFilePath) . " " . escapeshellarg($tempFilePath));
								break;
							case "png":
								// Use ImageMagick To Compress Image, Convert To Progressive, and Resize as Necessary
								exec("convert -strip -interlace line -quality " . escapeshellarg($quality) . "% -resize " . $maxWidth . "x" . $maxHeight . " " . escapeshellarg($tempFilePath) . " " . escapeshellarg($tempFilePath));
								// Use OptiPNG To Compress Image
								exec("optipng -strip all -backup " . escapeshellarg($path) . " -clobber " . escapeshellarg($path));
								break;
							default:
								$imageAlter = new MOXMAN_Media_ImageAlter();
								$imageAlter->load($tempFilePath);
								$imageAlter->resize($maxWidth, $maxHeight, true);
								$imageAlter->save($tempFilePath, $quality);
								break;
						}
					} else {
						switch (MOXMAN_Util_PathUtils::getExtension($tempFilePath)) {
							...
							case "jpeg":
								...
								exec("convert -strip -interlace line -quality " . escapeshellarg($quality) . "% " . escapeshellarg($tempFilePath) . " " . escapeshellarg($tempFilePath));
								break;
							case "png":
								// Use OptiPNG To Compress Image
								exec("optipng -strip all -backup " . escapeshellarg($path) . " -clobber " . escapeshellarg($path));

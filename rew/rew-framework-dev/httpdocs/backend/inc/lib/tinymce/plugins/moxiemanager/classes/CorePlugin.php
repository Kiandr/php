	...
	public function createThumbnail(MOXMAN_Vfs_IFile $file, $localTempFile = null) {
		...
		switch (MOXMAN_Util_PathUtils::getExtension($path)) {
			...
			case "jpeg":
				...
				exec("convert -strip -interlace line -quality " . escapeshellarg($config->get('thumbnail.jpeg_quality')) . "% -resize " . $config->get('thumbnail.width') . "x" . $config->get('thumbnail.height') . " " . escapeshellarg($path) . " " . escapeshellarg($thumbnailFile->getInternalPath()));
				break;
			case "png":
				// Use ImageMagick To Compress Image, Convert To Progressive, and Resize as Necessary
				exec("convert -strip -interlace line -quality " . escapeshellarg($config->get('thumbnail.jpeg_quality')) . "% -resize " . $config->get('thumbnail.width') . "x" . $config->get('thumbnail.height') . " " . escapeshellarg($path) . " " . escapeshellarg($thumbnailFile->getInternalPath()));
				// Use OptiPNG To Compress Image
				exec("optipng -strip all -backup " . escapeshellarg($thumbnailFile->getInternalPath()) . " -clobber " . escapeshellarg($thumbnailFile->getInternalPath()));
				break;
			default:
				if ($localTempFile) {

<form action="?submit" method="post" class="rew_check">
    ...
    <div class="bar">
        <div class="bar__title">Add New Development</div>
        <div class="bar__actions">
            <a class="bar__action" href="../">
                <svg class="icon">
    ...
    <div class="block">
        ...
        <h3 class="panel__hd">Development Details</h3>
        <div class="cols">
            <div class="field col w1/2">
                <label class="field__label">Development Title <em class="required">*</em></label>
                <input class="w1/1" data-bind="input[name=link]" name="title" value="<?=htmlspecialchars($_POST['title']); ?>" autofocus required>
            </div>

            <div class="field col w1/2">
                <label class="field__label">Sub-Title</label>
                <input class="w1/1" type="text" name="subtitle" value="<?=Format::htmlspecialchars($_POST['subtitle']); ?>">
            </div>
        </div>
        <div class="field">
            <label class="field__label">Development Link <em class="required">*</em></label>
            <input class="w1/1" name="link" value="<?=Format::slugify($_POST['link']); ?>" data-slugify required>
            <p class="text--mute">
        ...
        <div class="field">
            ...
            <textarea class="w1/1" rows="6" name="description"><?=Format::htmlspecialchars($_POST['description']); ?></textarea>
        </div>
        <div class="cols">
            <div class="field col w3/4">
                ...
                <?php
                    ...
                    echo '<select name="community_id" data-selectize=\'' . json_encode([
        ...
        <div class="field">
            ...
            <div class="">
                <label class="toggle">
                    <input<?=(($_POST['is_enabled'] == 'N') ? ' disabled' : ''); ?> type="radio" name="is_featured" value="Y"<?=($_POST['is_featured'] != 'N') ? ' checked' : ''; ?>>
                    <span class="toggle__label">Yes</span>
                </label>
                <label class="toggle">
                    <input<?=(($_POST['is_enabled'] == 'N') ? ' disabled' : ''); ?> type="radio" name="is_featured" value="N"<?=($_POST['is_featured'] == 'N') ? ' checked' : ''; ?>>
                    <span class="toggle__label">No</span>
                </label>
            </div>
        </div>
        ...
        foreach ($fieldsets as $fieldset) {
            ...
            if (!empty($fieldset['fields'])) {
                foreach ($fieldset['fields'] as $name => $field) {
                    $extra = '';
                    $input = $field['type'];
                    $value = isset($_POST[$name]) ? Format::htmlspecialchars($_POST[$name]) : NULL;
                    // Placeholder text
        ...
        <div class="field">
            ...
            <div id="search_criteria_toggle">
                <label class="toggle toggle--stacked" for="search_criteria_builder">
                    <input type="radio" name="search_criteria" id="search_criteria_builder" value="builder"<?=(!in_array($_POST['search_criteria'], ['snippet', 'disabled']) ? ' checked' : ''); ?>>
                    <span class="toggle__label">Search Criteria</span>
                </label>
                <label class="boolean toggle toggle--stacked" for="search_criteria_snippet">
                    <input type="radio" name="search_criteria" id="search_criteria_snippet" value="snippet"<?=($_POST['search_criteria'] == 'snippet' ? ' checked' : ''); ?>>
                    <span class="toggle__label">IDX Snippet</span>
                </label>
                <label class="boolean toggle toggle--stacked" for="search_criteria_disabled">
                    <input type="radio" name="search_criteria" id="search_criteria_disabled" value="disabled"<?=($_POST['search_criteria'] == 'disabled' ? ' checked' : ''); ?>>
                    <span class="toggle__label">No Listings</span>
                </label>
            </div>
        </div>

        <div id="criteria-snippet" class="field<?=($_POST['search_criteria'] != 'snippet' ? ' hidden' : ''); ?>">
            <div class="field">
                ...
                <select class="w1/1" name="idx_snippet_id">
                    ...
                    <?php
                        ...
                        if (!empty($idx_snippets) && is_array($idx_snippets)) {
                            $idx_snippet_id = $_POST['idx_snippet_id'];
        ...
        <div id="criteria-builder" class="field<?=($_POST['search_criteria'] == 'snippet' ? ' hidden' : ''); ?>">
            <?php
        ...
        <div class="field">
            <div data-uploader='{ "extraParams": { "type": "development" }}'>

<form id="idx-builder-form" action="?submit" method="post" class="rew_check">
    ...
    <div class="bar">
        <div class="bar__title"><?=Format::htmlspecialchars($community['title']); ?></div>
        <div class="bar__actions">
            <a href="/backend/cms/tools/communities/" class="bar__action">
                <svg class="icon">
    ...
    <div class="block">
        ...
        <div class="field">
            ...
            <input class="w1/1" type="text" name="title" value="<?=Format::htmlspecialchars($community['title']); ?>" autofocus required>
        </div>

        <div class="field">
            <label class="field__label"><?= __('Sub-Title'); ?></label>
            <input class="w1/1" type="text" name="subtitle" id="subtitle" value="<?=Format::htmlspecialchars($community['subtitle']); ?>">
        ...
        <div class="field">
            ...
            <input class="w1/1" type="text" name="snippet" value="<?=Format::htmlspecialchars($community['snippet']); ?>" data-slugify required>
        </div>
            ...
            if (!empty($can_tag)) {
                ...
                if (!empty($community_tags) && is_array($community_tags)) {
                    foreach ($community_tags as $community_tag) {
                        $selected = is_array($community['tags']) && in_array($community_tag, $community['tags']) ? ' selected' : '';
            ...
            if (!empty($page_select)) {
                ...
                if (!empty($cms_pages) && is_array($cms_pages)) {
                    ...
                    foreach ($cms_pages as $cms_page) {
                        $selected = $community['page_id'] == $cms_page['page_id'] ? ' selected' : '';
                ...
                if (!empty($cms_subpages) && is_array($cms_subpages)) {
                    foreach ($cms_subpages as $optgroup => $subpages) {
                        ...
                        foreach ($subpages as $cms_subpage) {
                            $selected = $community['page_id'] == $cms_subpage['page_id'] ? ' selected' : '';
        ...
        <?php if (!empty($has_video_link_feature)) { ?>
            <div class="field">
                ...
                <input class="w1/1" name="video_link" value="<?=Format::htmlspecialchars($community['video_link']); ?>">
            </div>
        <?php } ?>
        ...
        <div class="field">
            ...
            <label class="toggle">
                <input type="radio" name="is_enabled" value="Y"<?=($community['is_enabled'] == 'Y') ? ' checked' : ''; ?>>
                <span class="toggle__label"><?= __('Yes'); ?></span>
            </label>
            <label class="toggle">
                <input type="radio" name="is_enabled" value="N"<?=($community['is_enabled'] != 'Y') ? ' checked' : ''; ?>>
        ...
        <?php if ($nonelite_headings) { ?>
            ...
            <div class="field">
                ...
                <input class="w1/1" type="text" name="stats_heading" value="<?=Format::htmlspecialchars($community['stats_heading']); ?>">
        ...
        <div class="field">
            ...
            <input class="w1/1" type="text" name="stats_total" value="<?=Format::htmlspecialchars($community['stats_total']); ?>">
        </div>

        <div class="field">
            <label class="field__label"><?= __('Average Price'); ?></label>
            <input class="w1/1" type="text" name="stats_average" value="<?=Format::htmlspecialchars($community['stats_average']); ?>">
        </div>
        ...
        <?php if ($nonelite_headings) { ?>
            ...
            <div class="field">
                ...
                <input class="w1/1" type="text" name="stats_highest" value="<?=Format::htmlspecialchars($community['stats_highest']); ?>">
            </div>

            <div class="field">
                <label class="field__label"><?= __('Lowest Listing Price'); ?></label>
                <input class="w1/1" type="text" name="stats_lowest" value="<?=Format::htmlspecialchars($community['stats_lowest']); ?>">
            </div>
        ...
        <?php if (!empty($anchor_links)) { ?>
            ...
            <div class="field">
                ...
                <input class="w1/1" type="text" name="anchor_one_text" value="<?=Format::htmlspecialchars($community['anchor_one_text']); ?>">
                <label class="hint"><?= __('(Default: Community Summary)'); ?></label>
            </div>

            <div class="field">
                <label class="field__label"><?= __('Right Anchor Text'); ?></label>
                <input class="w1/1" type="text" name="anchor_two_text" value="<?=Format::htmlspecialchars($community['anchor_two_text']); ?>">
                <label class="hint"><?= __('(Default: Homes for Sale)'); ?></label>
            </div>

            <div class="field">
                <label class="field__label"><?= __('Left Anchor Link'); ?></label>
                <input class="w1/1" type="text" name="anchor_one_link" value="<?=Format::htmlspecialchars($community['anchor_one_link']); ?>">
                <label class="hint"><?= __('(Default: #community-summary)'); ?></label>
            </div>

            <div class="field">
                <label class="field__label"><?= __('Right Anchor Link'); ?></label>
                <input class="w1/1" type="text" name="anchor_two_link" value="<?=Format::htmlspecialchars($community['anchor_two_link']); ?>">
                <label class="hint"><?= __('(Default: #homes-for-sale)'); ?></label>
            </div>
        ...
        <div class="field" id="search_criteria_toggle">
            <div class="-marB8">
                <label class="toggle">
                    <input type="radio" name="search_criteria" value="true"<?=(empty($community['idx_snippet'])) ? ' checked' : ''; ?>>
                    <span class="toggle__label"><?= __('Use Search Criteria'); ?></span>
                </label>
            </div>
            <div class="-marB8">
                <label class="toggle">
                    <input type="radio" name="search_criteria" value="false"<?=(!empty($community['idx_snippet'])) ? ' checked' : ''; ?>>
                    <span class="toggle__label"><?= __('Use IDX Snippet'); ?></span>
                </label>
            </div>
        ...
        <div id="idx_snippet_panel" class="<?=(empty($community['idx_snippet'])) ? 'hidden' : ''; ?>">
            <div class="field">
                <label class="field__label"><?= __('IDX Snippet'); ?></label>
                <select class="w1/1" name="idx_snippet">
                    <option value="" <?=(empty($community['idx_snippet']) ? ' selected' : ''); ?>><?= __('Select IDX Snippet'); ?>...</option>
                    <?php foreach ($idx_snippets as $idx_snippet) { ?>
                        <option value="<?=$idx_snippet['id']; ?>" <?=($community['idx_snippet'] == $idx_snippet['id'] ? ' selected' : ''); ?>>
        ...
        <div id="search_criteria_panel" class="<?=(!empty($community['idx_snippet'])) ? 'hidden' : ''; ?>">
            <?php
        ...
        <h3 class="panel__hd text"><?= __('Community Photos'); ?></h3>
        ...
        <div class="field">
            <div data-uploader='<?=json_encode([
                'inputName' => 'uploads[]',
                'extraParams' => ['type' => 'community', 'row' => (int) $community['id']]
            ]); ?>'>

...
class ShowingEventFactoryTest extends \Codeception\Test\Unit
{
    ...
    /**
     * @covers \REW\Backend\Dashboard\EventFactory\FormEvents\ShowingEventFactory::__construct()
     * @covers \REW\Backend\Dashboard\AbstractEventFactory::__construct()
     */
    public function testContruct()
    {
        $showingEventFactory = new ShowingEventFactory($this->db, $this->auth, $this->settings, $this->format, $this->cache, $this->idxFactory);
        $this->assertInstanceOf('REW\Backend\Dashboard\EventFactory\FormEvents\ShowingEventFactory', $showingEventFactory);
    }

    /**
     * @covers \REW\Backend\Dashboard\EventFactory\FormEvents\ShowingEventFactory::getMode()
     * @covers \REW\Backend\Dashboard\AbstractEventFactory::getMode()
     */
    public function testGetMode()
    {
        $showingEventFactory = new ShowingEventFactory($this->db, $this->auth, $this->settings, $this->format, $this->cache, $this->idxFactory);
        $this->assertEquals('showing', $showingEventFactory->getMode());
    }

    /**
     * @covers \REW\Backend\Dashboard\EventFactory\FormEvents\ShowingEventFactory::getEvent()
    ...
    public function testGetEvent()
    {
        ...
        $stmt->shouldReceive('fetch')
            ->once()
            ->andReturn(['user_id' => 17, 'first_name' => 'Matthew', 'last_name' => 'Brown', 'email' => 'test@realestatewebmasters.com', 'phone_cell' => '250 111 1111', 'image' =>  null, 'agent' => 4, 'status' => 'pending', 'form_id' => 8, 'form' => 'IDX Showing', 'data' => 'a:5:{s:4:"form";s:16:"Property Showing";s:10:"ListingMLS";s:10:"17208692PS";s:11:"ListingType";s:6:"Rental";s:11:"ListingFeed";s:6:"carets";s:8:"comments";s:71:"Please call me, I would like to see this listing in person on Thursday.";}', 'showing' => 'This is a test inquiry.']);
        ...
        $showingEventFactory = new ShowingEventFactory($this->db, $this->auth, $this->settings, $this->format, $this->cache, $this->idxFactory);
        $event = $showingEventFactory->getEvent($eventId);
        ...
        $this->assertEquals('showing', $event['mode']);
        ...
        $this->assertEquals('IDX Showing', $event['data']['form']['name']);
        $this->assertEquals('17208692PS', $event['data']['form']['mls_number']);
        $this->assertEquals('Rental', $event['data']['form']['type']);
        $this->assertEquals('carets', $event['data']['form']['feed']);
        $this->assertEquals('Please call me, I would like to see this listing in person on Thursday.', $event['data']['form']['comments']);
    }
}
